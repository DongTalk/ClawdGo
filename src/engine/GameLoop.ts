...
export class GameLoop {
  ...
  private async processResponsePhase(gameState: GameState, player: Player): Promise<void> {
    ...
    if (result.success) {
      // 更新游戏状态
      this.config.gameStateManager.updateGameState(result.modifiedGameState);
  ...
  private async processDiscardPhase(gameState: GameState, player: Player): Promise<void> {
    // R4.3: 根据轮次数获取手牌上限（12轮次制）
    // 1-4轮次：3张，5-8轮次：4张，9-12轮次：5张
    // 注意：使用 gameState.round（轮次）而不是 gameState.turn（回合）
    let handLimit: number;
    if (gameState.round <= 4) {
      handLimit = 3;
    } else if (gameState.round <= 8) {
      handLimit = 4;
    } else {
      handLimit = 5;
    }
