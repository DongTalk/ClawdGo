...
- **修复压缩导致的关键行为缺失**：恢复 `clawdgo` 唤醒先出导航菜单（含版权尾部）、再问称呼的强约束
- **修复模式边界漂移**：W 模式保持显式触发，禁止默认进入；新增“跨会话旧上下文不可信”约束，避免误判“仍在 B 模式”
- **修复 H 模式操作一致性**：统一 `duel` 子命令规则，保留“指令即执行许可 + 返回真实结果”行为

### 调整 / Changed

- **local-first 基线**：v1.2.1 不再依赖 `soul.md` 作为运行前提，`memory/reset/uninstall` 仅作用于当前会话状态
- **SKILL 指令结构优化**：内部规则改为英文主导、用户可见模板保持中文，降低提示词体积并提升稳定性
- **references 去污染**：`w-mode-rules.md` 与 `h-mode-ops.md` 去除 soul 依赖表述；`soul-format.md` 改为停用声明
