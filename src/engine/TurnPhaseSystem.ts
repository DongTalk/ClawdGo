...
import { getDrawCountByRound } from '@/types/gameConstants';
...
export class TurnPhaseSystem {
   ...
   * - 检查手牌上限
   * - 弃置超额手牌
   * 手牌上限基于轮次（round）而非回合（turn）：
   * 1-4轮次为3张，5-8轮次为4张，9-12轮次为5张（12轮次制）
   */
  private static executeDiscardPhase(
    ...
    // 12轮次制：1-4轮次3张，5-8轮次4张，9-12轮次5张
    const currentRound = gameState.round;
    let maxHandSize: number;
