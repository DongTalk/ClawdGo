...
export function createGame(
  ...
  const players: Player[] = playerConfigs.map((config) => {
    ...
    return {
      ...
      team: config.faction === 'attacker' ? 'A' : 'B', // 根据阵营分配队伍
      resources: {
...
export function advancePhase(gameState: GameState): GameState {
  ...
  if (nextPhase === 'draw') {
    ...
    newState.players = newState.players.map((player, index) => {
      ...
      // 计算需要抽的牌数（考虑动态手牌上限）
      const drawCount = getDrawCountByRound(currentRound);
      const currentHandSize = player.hand.length;
      const actualDrawCount = Math.min(drawCount, handLimit - currentHandSize);
    ...
    newState.log = [...newState.log, {
      ...
      playerId: currentPlayer.id,
      action: 'draw_cards',
      message: `[${currentPlayer.name}] 摸牌阶段：抽了${totalDrawn}张牌（手牌上限${handLimit}张）`,
      details: {
        count: totalDrawn,
        handLimit: handLimit
      }
