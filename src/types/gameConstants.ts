...
/** 手牌限制 (R4.3) - 12轮次制调整（原：24回合制） */
export const HAND_LIMITS = {
  /** 初始手牌数量 */
  INITIAL: 4,
  /** 1-4轮次手牌上限 (早期游戏，原：1-8回合) */
  EARLY_GAME_MAX: 3,
  /** 5-8轮次手牌上限 (中期游戏，原：9-16回合) */
  MID_GAME_MAX: 4,
  /** 9-12轮次手牌上限 (后期游戏，原：17-24回合) */
  LATE_GAME_MAX: 5,
  /** 每轮次抽牌数 - 1-4轮次3张，5-8轮次4张，9-12轮次5张（原：固定4张） */
  DRAW_PER_ROUND: {
    EARLY: 3,  // 1-4轮次
    MID: 4,    // 5-8轮次
    LATE: 5,   // 9-12轮次
...
/**
 * 根据轮次数获取手牌上限 - 12轮次制（原：24回合制）
 * @param round 当前轮次数（原：回合数）
 * @returns 手牌上限
 */
export function getHandLimitByRound(round: number): number {
  // R4.3: 1-4轮次手牌上限3张，5-8轮次手牌上限4张，9-12轮次手牌上限5张（原：1-8/9-16/17-24）
  if (round <= 4) return HAND_LIMITS.EARLY_GAME_MAX;
  if (round <= 8) return HAND_LIMITS.MID_GAME_MAX;
  return HAND_LIMITS.LATE_GAME_MAX;
}

// 保留旧函数名以兼容现有代码，但内部使用轮次
/** @deprecated 请使用 getHandLimitByRound */
export function getHandLimitByTurn(turn: number): number {
  return getHandLimitByRound(turn);
}

/**
 * 根据轮次数获取抽牌数 - 12轮次制（原：固定每回合4张）
 * @param round 当前轮次数
 * @returns 抽牌数
 */
export function getDrawCountByRound(round: number): number {
  // R4.3: 1-4轮次抽3张，5-8轮次抽4张，9-12轮次抽5张
  if (round <= 4) return HAND_LIMITS.DRAW_PER_ROUND.EARLY;
  if (round <= 8) return HAND_LIMITS.DRAW_PER_ROUND.MID;
  return HAND_LIMITS.DRAW_PER_ROUND.LATE;
}

// 保留旧函数名以兼容现有代码
/** @deprecated 请使用 getDrawCountByRound */
export function getDrawCountByTurn(turn: number): number {
  return getDrawCountByRound(turn);
}
